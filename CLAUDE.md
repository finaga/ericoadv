...
Plain HTML + embedded CSS + a little vanilla JS. **No build step, no framework, no dependencies.** Each deployed page is a self-contained file in `Web/` (all CSS in a `<style>` in the head, all JS in one inline `<script>`). **One sanctioned exception:** `Web/hubbar.js` is a single shared front-end asset — the cross-page "preview bar" — referenced via `<script src="hubbar.js" data-…>` on every deliverable page (it's the only shared `.js`; everything else stays inline).

| File | Role |
|------|------|
| `Web/site.html` | **The firm site.** Single page; the main deliverable. |
| `Web/v2.html` | **Alternative concept "Edição Noturna"** (Jun 2026) — cinematic dark-first version of the same single page, same copy/assets verbatim. Linked from the hub as card 04 "O Site — Versão B". GSAP 3.12 + ScrollTrigger + Lenis + Three.js r149 via CDN (the only pages with JS libs; everything else stays dependency-free). WebGL navy-duotone hero (offscreen-canvas → `CanvasTexture` upload — direct `<img>` textures fail on some GL stacks), Lenis smooth scroll, scroll-morphing body background (sombra → azul → concreto), pinned horizontal Publicações gallery (≥901px; swipe rail below), char-split hero (word-wrapped spans — char spans alone break mid-word), magnetic contact CTA. Hero CTA sits *below* the sub-copy (left-aligned), not in a row beside it. Headline reveal fires 0.34s into the loader exit (via `gsap.delayedCall`) so the char-rise clears the lifting loader panel and is actually visible. **No grain overlay, no marquee, native cursor** — all three were removed after review (grain read as static/error, marquee read as AI-generic, custom cursor unwanted). Giant section numbers are solid tonal dourado, not outlined stroke. All motion behind `prefers-reduced-motion`; no-JS/no-GSAP fallbacks show full content. Hero type is `clamp(34px, 5.25vw, 76px)` — char-split Syne 800 measures ~6% wider than normal text (kerning lost); 76px is the one-line-per-statement ceiling at 1440. |
| `Web/index.html` | Project hub. **Five cards**: 01 Estratégia · 02 Manual · 03 O Site — Versão A (`site.html`) · 04 O Site — Versão B (`v2.html`, dark accent) · 05 Downloads. |
| `Web/hubbar.js` | **Shared "preview bar"** (Jun 2026) — slim fixed top bar (44px) with a *Voltar ao hub* button + the current section name (Site A · Site B · Downloads · Estratégia · Manual da Marca). It **pushes content down**, never floats over it: injects `html.has-hubbar body{padding-top:var(--hubbar-h)!important}` and offsets each page's own top:0 chrome. Self-injecting: each page adds `<script src="hubbar.js?v=N" data-section="…" data-offset="<selectors>" [data-shift="<selectors>"]>`. **The `?v=N` token is a cache-bust — bump it on EVERY edit to `hubbar.js`**, otherwise browsers/CDN serve the stale shared script (the `src` is the only thing whose change forces a refetch). Two offset modes: `data-offset` snaps top:0 full-width/full-height chrome flush to the bar bottom (`top:var(--hubbar-h)` — the site navs + `.progress`, the downloads/manual `nav.side`, estrategia's `nav`); `data-shift` moves an *inset corner pill* down by the bar height via `margin-top` so it keeps its original gap (the downloads/manual `.toggle` "Modo escuro", which would otherwise touch the bar). **Shows on every staging/preview host; hidden only on the production hosts `erico.law` / `www.erico.law`** (mirrors vercel.json's host-scoped noindex) — so the public firm site never shows it. `?nohub` force-hides for screenshots. It is the project's single back-to-hub control: the per-page *← Início* buttons (estrategia `.nav-back`, downloads/guideline `.back-home`) were **removed** in its favor. Reveals after the page's own loader clears (`window.load` + 220ms; 2.6s fallback) and calls `ScrollTrigger.refresh()` since the added body padding shifts layout. Page-specific accommodation lives in each page's own CSS (e.g. `html.has-hubbar .hero{min-height:calc(100svh - var(--hubbar-h))}` on the two site pages). |
