...
| `Web/index.html` | Project hub. **Five cards**: 01 Estratégia · 02 Manual · 03 O Site — Versão A (`site.html`) · 04 O Site — Versão B (`v2.html`, dark accent) · 05 Downloads. On load it sets `sessionStorage.ericoHub='1'` so deliverable pages know the visitor came through the hub (drives `hubbar.js`). |
| `Web/hubbar.js` | **Shared "preview bar"** (Jun 2026) — slim fixed top bar with a *Voltar ao hub* button + the current section name (Site A · Site B · Downloads · Estratégia · Manual da Marca). Self-injecting: each page adds `<script src="hubbar.js" data-section="…" data-offset="<selectors>" [data-hide="<selectors>"]>`. `data-offset` = top:0 chrome to push down by the bar height (`--hubbar-h` = 44px); `data-hide` = redundant chrome to remove (downloads/guideline's own `.back-home`). **Shows only when arrived via the hub** — gate is `sessionStorage.ericoHub==='1'` OR a `?hub` query param (the param is an explicit deep-link override). So the production firm site at erico.law (reached directly, no hub) never shows it. Reveals after the page's own loader clears (`window.load` + 220ms; 2.6s fallback) and calls `ScrollTrigger.refresh()` since the added body padding shifts layout. Page-specific accommodation lives in each page's own CSS (e.g. `html.has-hubbar .hero{min-height:calc(100svh - var(--hubbar-h))}` on the two site pages). |
